...
$query = "SELECT o.order_id, o.tracking_code, o.customer_name, o.services_requested, o.supplies_requested, o.customer_note, o.status as order_status, o.final_price,
                 pl.load_id, pl.bag_label, pl.status
          FROM `Order` o
                                        ...
                                        <?php if (count($group['orders']) > 0): ?>
                                            <?php foreach ($group['orders'] as $order_id => $order):
                                                ...
                                                foreach ($order['loads'] as $l) {
                                                    ...
                                                    if ($l['status'] === 'Ready for Pickup') $awaitingCount++;
                                                }
                                            ?>
                                                            ...
                                                            <?php else: ?>
                                                                <?php foreach ($order['loads'] as $load): ?>
                                                                    <div class="bag-item border rounded p-3 mb-2 shadow-sm">
                                                                        ...
                                                                        <?php if ($isOpen && $s !== 'Completed' && $s !== 'Cancelled'): ?>
                                                                            <div class="mt-2">
                                                                                <div class="bg-light p-2 rounded border">
                                                                                    <?php
                                                                                    // Dynamically build the allowed phases based on requested services
                                                                                    ...
                                                                                    $phases = ['Pending Dropoff', 'In Queue'];
                                                                                    if ($hasW) $phases[] = 'Washing';
                                                                                    if ($hasD) $phases[] = 'Drying';
                                                                                    if ($hasF) $phases[] = 'Folding';
                                                                                    $phases[] = 'Ready for Pickup';
                                                                                    $phases[] = 'Completed';

                                                                                    $currentIndex = array_search($s, $phases);
                                                                                    $prevPhase = ($currentIndex > 0) ? $phases[$currentIndex - 1] : null;
                                                                                    $nextPhase = ($currentIndex !== false && $currentIndex < count($phases) - 1) ? $phases[$currentIndex + 1] : null;
                                                                                    ?>
                                                                                    <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">

                                                                                        <?php if ($prevPhase && !in_array($s, ['Pending Dropoff', 'Completed', 'Cancelled'])): ?>
                                                                                            <form action="backend/update_status.php" method="POST" class="m-0" onsubmit="submitNextPhase(event, this)">
                                                                                                <input type="hidden" name="is_ajax" value="1">
                                                                                                <input type="hidden" name="action" value="update_phase">
                                                                                                <input type="hidden" name="load_id" value="<?php echo $load['load_id']; ?>">
                                                                                                <input type="hidden" name="target_phase" value="<?php echo $prevPhase; ?>">
                                                                                                <button type="submit" class="btn btn-outline-secondary btn-sm fw-bold shadow-sm">
                                                                                                    <i class="bi bi-arrow-left-short"></i> Back to <?php echo $prevPhase; ?>
                                                                                                </button>
                                                                                            </form>
                                                                                        <?php else: ?>
                                                                                            <div></div> <?php endif; ?>

                                                                                        <?php if ($nextPhase && !in_array($s, ['Pending Dropoff', 'Ready for Pickup', 'Completed', 'Cancelled'])): ?>
                                                                                            <form action="backend/update_status.php" method="POST" class="m-0" onsubmit="submitNextPhase(event, this)">
                                                                                                <input type="hidden" name="is_ajax" value="1">
                                                                                                <input type="hidden" name="action" value="update_phase">
                                                                                                <input type="hidden" name="load_id" value="<?php echo $load['load_id']; ?>">
                                                                                                <input type="hidden" name="target_phase" value="<?php echo $nextPhase; ?>">
                                                                                                <button type="submit" class="btn btn-info text-dark btn-sm fw-bold shadow-sm border border-info">
